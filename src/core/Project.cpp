...
//        }
        ...
        /*  For Jack it appears that the saving and loading of "AudioBuses" is not necessary.
         *  The buses are created from scratch each time a project is loaded from
         *  Track::connect_to_jack() using the track information of the project.tpf file.
         *  If we load these AudioBuses here and add them to the m_softwareAudioBuses,
         *  The we get duplicates when Track::connect_to_jack()
         *  calls Project::create_software_audio_bus() which creates new buses. The 
         *  duplicates are saved with each file save in the project.tpf and with each
         *  load, new ones are created. The project.tpf file then accumulates new phantom
         *  "AudioBuses" with each save and load for every track, endlessly...!
         *  The solution here is to simply ignore all "AudioBuses" and rely on the call
         *  to Track::connect_to_jack() to create them. This will also eliminate any old
         *  accumulated phantom buses. It does not appear that m_hardwareAudioBuses
         *  are used, since Jack handles the hardware devices.
         *  So, we are bypassing the "AudioBuses" section here for Jack.
         *  Also eliminating the error messages generated by TSend::set_state()
         *  and Track::set_state() when they cannot find the buses for Jack.
